...
VariableKey = NamedTuple("VariableKey", [("operation", ir.OpView), ("type", VariableType), ("index", int)])
...
def find_assignments_for(
  ...
  remaining_unknowns = set(equation_system.assignments.keys()) - unknowns
...
def _add_equation_system_derivation_rule(
    op: type[ir.OpView], rule: EquationSystemDerivationRule
):
  if op is not None:
...
@partial(_add_equation_system_derivation_rule, arith.ConstantOp)
...
def _ensure_right_number_of_layouts(
  ...
  if len(in_layouts) != len(list(filter(is_array, op.operands))):
    raise ValueError(
        "Expected the same number of in_layouts as vector operands."
    )
  if len(out_layouts) != len(list(filter(is_array, op.results))):
    raise ValueError(
...
def assign_layouts(solution: dict[Variable, equations.ConstantExpression]):
  ...
  solution_sorted_by_op = sorted(
      solution.items(), key=lambda kv: hash(kv[0].key.operation)
  )
